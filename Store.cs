...
namespace LemonadeStand_3DayStarter
{
    class Store
    {
        ...
        public void StoreStart(Player player)
        {
            ...
            switch (userInput)
            {
                case "buy":
                    ...
                    int sugarCube = PlayerBuySugar(player);
                    AddSugarToInventory(player, sugarCube);
                    int iceCube = PlayerBuyIce(player);
                    AddIceToInventory(player, iceCube);
                    int cup = PlayerBuyCup(player);
                    AddCupToInventory(player, cup);
                    player.inventory.DisplayInventory();
                    GetPlayerTotalExpense(lemon * new Lemon().price, sugarCube * new SugarCube().price, iceCube * new IceCube().price, cup * new Cup().price);
                    Console.Clear();
                    StoreStart(player);
                    break;
        ...
        public int PlayerBuySugar(Player player)
        {
            ...
            int sugarCube = int.Parse(Console.ReadLine());
            if (!WithdrawSugarMoney(player, GetSugarExpense(sugarCube)))
            {
                return PlayerBuySugar(player);
            }
            return sugarCube;
        }
        public double GetSugarExpense(int sugarCube)
        {
            double expense;
            expense = sugarCube * new SugarCube().price;
            return expense;
        }
        ...
        public void AddSugarToInventory(Player player, int sugar)
        {
            ...
            {
                player.inventory.AddSugar(new SugarCube());
            }
        }
        ...
        public void AddIceToInventory(Player player, int ice)
        {
            for (int i = 0; i < ice; i++)
            {
                player.inventory.AddIce(new IceCube());
            }
        }
